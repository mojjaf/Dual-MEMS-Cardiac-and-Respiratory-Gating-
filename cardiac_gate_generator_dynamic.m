function [ LM_5bin,LM_3bin ] = cardiac_gate_generator_dynamic( signal, fd_points,fs,gr )
%UNTITLED7 Summary of this function goes here
%   Detailed explanation goes here
win=20 ;%in second
step=20;
fd_diastole=[];
[heartRate,systolicTimeInterval] = getSTIs(signal,win*fs,fs);
...
trailingsamples = mod(length(tmp_signal), windowLength);

sampleframes = reshape( tmp_signal(1:end-trailingsamples), windowLength, []);
mod_frame=numel(sampleframes):1:numel(sampleframes)+trailingsamples;

% previous_frame=0;
% for i=1:numOfFrames % for each frame
%     tmp=(curPos:curPos+windowLength-1);
%     sig_frame = tmp(ismember(tmp, fd_points));
% %     sig_frame = sig_frame(~ismember(sig_frame, previous_frame));
%     temp_dias(1:numel(sig_frame))=systolicTimeInterval(i);
%     fd_diastole=[fd_diastole temp_dias];
%        curPos = curPos + step;
% %     previous_frame=sig_frame;
% end
for i=1:numel(fd_points) % for each cardiac cycle
    if fd_points(i)>numel(sampleframes)
        fd_diastole(i)=systolicTimeInterval(end);
    else
    [~,frame_ID]=find(sampleframes==fd_points(i));
    fd_diastole(i)=systolicTimeInterval(frame_ID);
    end
end
...
MEMS_bin(i,j)=MEMS_bin(i,j-1)+(fd_diastole(i)*fs/4);
bin_num(i,j)=j;
end
